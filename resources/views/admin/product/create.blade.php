...
@section('content')
    ...
    <div class="flex items-center gap-4 py-2 border-b border-slate-100 dark:border-slate-800">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">Daftarkan obat, vitamin, atau suplemen baru ke sistem
                katalog.</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="sm:col-span-2">
                            <label for="prodName"
                                ...
                                Obat / Produk</label>
                            <input type="text" id="prodName" name="name" required value="{{ old('name') }}"
                                placeholder="Contoh: Apoquel 16mg / Flunixin Injeksi"
                        ...
                        <div>
                            <label for="prodBrand"
                                ...
                                / Produsen</label>
                            <input type="text" id="prodBrand" name="brand" value="{{ old('brand') }}"
                                placeholder="Contoh: Zoetis / Pfizer Animal Health"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="prodPrice" name="price" required value="{{ old('price') }}"
                        ...
                        <div>
                            <label for="prodStock"
                                class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Stok
                                Awal</label>
                            <input type="number" id="prodStock" name="stock" required value="{{ old('stock', 0) }}"
                ...
                <div
                    ...
                    <div>
                        ...
                        <div
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs font-bold text-slate-600 dark:text-slate-300">Klik untuk upload foto
                                    produk</p>
                    ...
                    <div>
                        ...
                        <textarea id="prodDesc" name="description" rows="5" required
                            placeholder="Tuliskan spesifikasi lengkap, khasiat dasar, dan mekanisme obat..."
            ...
            <div class="lg:col-span-5 space-y-6">
                ...
                {{-- <div
                    ...
                    <div
                        class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                        <div class="flex items-center h-5">
                            <input id="needsPrescription" name="needs_prescription" value="1" type="checkbox"
                                {{ old('needs_prescription') ? 'checked' : '' }}
                                class="h-4.5 w-4.5 rounded border-slate-300 dark:border-slate-700 text-emerald-600 focus:ring-emerald-500 cursor-pointer" />
                        </div>
                        <div class="text-xs">
                            <label for="needsPrescription"
                                class="font-bold text-slate-800 dark:text-white select-none cursor-pointer flex items-center gap-1.5">
                                Wajib Resep Dokter Hewan (R/)
                            </label>
                            <p class="text-slate-400 mt-1">Centang jika obat ini termasuk golongan obat keras yang
                                mewajibkan unggah berkas resep dari dokter hewan berizin.</p>
                        </div>
                    </div>
                            ...
                            @foreach (['Sapi', 'Kambing', 'Domba', 'Unggas', 'Anjing', 'Kucing', 'Hewan Eksotis'] as $animal)
                                <label class="cursor-pointer">
                                    <input type="checkbox" name="target_animals[]" value="{{ $animal }}"
                                        class="peer sr-only"
                                        {{ is_array(old('target_animals')) && in_array($animal, old('target_animals')) ? 'checked' : '' }} />
                                    <span
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="dosageForm" name="dosage_form"
                                ...
                                @foreach (['Tablet', 'Kapsul', 'Salep', 'Injeksi / Cairan Suntik', 'Sirup / Cairan Oral', 'Drop / Tetes', 'Powder / Serbuk'] as $form)
                                    <option value="{{ $form }}"
                                        {{ old('dosage_form') == $form ? 'selected' : '' }}>{{ $form }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="activeIngredients" name="active_ingredients"
                                value="{{ old('active_ingredients') }}"
                                placeholder="Contoh: Oclacitinib / Flunixin meglumine"
                                class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="batchNum"
                                ...
                                Batch / Lot</label>
                            <input type="text" id="batchNum" name="batch_number" value="{{ old('batch_number') }}"
                                placeholder="Contoh: B10294"
                                class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all" />
                        </div>
                        ...
                        <div>
                            <label for="expiryDate"
                                ...
                                Kadaluarsa</label>
                            <input type="date" id="expiryDate" name="expiry_date" value="{{ old('expiry_date') }}"
                                class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all" />
                        </div>
                    </div>
                </div> --}}
                ...
                <div
                    ...
                    <div>
                        ...
                        <textarea id="prodDosage" name="dosage_guidelines" rows="3"
                            placeholder="Cara pemberian (oral/injeksi), rute administrasi, dan dosis per kg berat badan..."
                            class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('dosage_guidelines') }}</textarea>
                    </div>
                    ...
                    <div>
                        <label for="prodIndication"
                            ...
                            Medis</label>
                        <textarea id="prodIndication" name="indication" rows="3"
                            placeholder="Penyakit target, gejala klinis yang ditangani, atau jenis mikroba target..."
                            class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('indication') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="prodNote" name="pharmacist_note" rows="3"
                            placeholder="Suhu penyimpanan ideal, kontraindikasi, efek samping, dan waktu henti obat (withdrawal time)..."
                            class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('pharmacist_note') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
