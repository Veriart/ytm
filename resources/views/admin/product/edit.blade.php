...
<form action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 mt-6">
    @csrf
    @method('PUT')
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-sm p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <!-- Stock -->
                    <div>
                        <label for="prodStock" class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Stok</label>
                        <input type="number" id="prodStock" name="stock" required value="{{ old('stock', $product->stock) }}" class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all @error('stock') border-rose-500 focus:ring-rose-500 @enderror" />
                        @error('stock')
                            <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-sm p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Foto Saat Ini</label>
                    <div class="mb-3">
                        @if(Str::startsWith($product->image, 'http'))
                            <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-32 h-32 rounded-xl object-cover border border-slate-200 dark:border-slate-700" />
                        @else
                            <img src="{{ asset($product->image) }}" alt="{{ $product->name }}" class="w-32 h-32 rounded-xl object-cover border border-slate-200 dark:border-slate-700" />
                        @endif
                    </div>
                    
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Ganti Foto Baru</label>
                    <div class="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-6 text-center hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors relative cursor-pointer group">
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs font-bold text-slate-600 dark:text-slate-300">Klik untuk upload foto baru</p>
                            <p class="text-[10px] text-slate-400">Biarkan kosong jika tidak ingin mengganti. Maks 2MB.</p>
                        </div>
                    </div>
                    @error('image')
                        <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p>
        ...
        <div class="lg:col-span-5 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-sm p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="prodDosage" name="dosage_guidelines" rows="3" placeholder="Cara pemberian, rute administrasi, dan dosis..." class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('dosage_guidelines', $product->dosage_guidelines) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="prodIndication" name="indication" rows="3" placeholder="Penyakit target, gejala klinis..." class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('indication', $product->indication) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="prodNote" name="pharmacist_note" rows="3" placeholder="Penyimpanan, kontraindikasi, withdrawal time..." class="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent dark:text-white transition-all">{{ old('pharmacist_note', $product->pharmacist_note) }}</textarea>
                </div>
            </div>
        </div>
    </div>
