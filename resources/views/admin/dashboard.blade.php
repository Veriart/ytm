...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">Ringkasan performa toko, inventaris kritis, dan peringatan
                obat hewan.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-6">
        <!-- Stat 1: Total Sales -->
        <div
            class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
            <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-1 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            {{-- <div
                class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-sm p-6">
                <div class="flex items-center justify-between pb-4 mb-4 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="inline-flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                ...
                <div class="h-64 relative w-full flex items-end">
                    <svg viewBox="0 0 500 200" class="w-full h-full overflow-visible">
                        ...
                        <line x1="0" y1="40" x2="500" y2="40" stroke="#f1f5f9" stroke-width="1"
                            class="dark:stroke-slate-700/50" />
                        ...
                        <line x1="0" y1="190" x2="500" y2="190" stroke="#e2e8f0"
                            stroke-width="1.5" class="dark:stroke-slate-700" />

                        <!-- Area Path (Filled gradient under line) -->
                        <path d="M 0 160 Q 50 140 100 130 T 200 90 T 300 120 T 400 60 T 500 45 L 500 190 L 0 190 Z"
                            fill="url(#chartGrad)" />

                        <!-- Line Path -->
                        <path d="M 0 160 Q 50 140 100 130 T 200 90 T 300 120 T 400 60 T 500 45" fill="none"
                            stroke="#10b981" stroke-width="3" stroke-linecap="round" />

                        <!-- Points -->
                        <circle cx="100" cy="130" r="4.5" fill="#ffffff" stroke="#10b981"
                            stroke-width="2.5" />
                        <circle cx="200" cy="90" r="4.5" fill="#ffffff" stroke="#10b981"
                            stroke-width="2.5" />
                        <circle cx="300" cy="120" r="4.5" fill="#ffffff" stroke="#10b981"
                            stroke-width="2.5" />
                        <circle cx="400" cy="60" r="4.5" fill="#ffffff" stroke="#10b981"
                            stroke-width="2.5" />
                        <circle cx="500" cy="45" r="4.5" fill="#ffffff" stroke="#10b981"
                            stroke-width="2.5" />
                    </svg>
                </div>
            ...
            <div
                class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <div
                            class="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                    ...
                    <a href="{{ route('admin.transaction.index') }}"
                        class="text-xs font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 flex items-center gap-1">
                        Lihat Semua
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr
                                class="bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 text-xs font-bold uppercase tracking-wider">
                            ...
                            @forelse($recentTransactions as $tx)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-200 flex items-center justify-center font-bold text-xs">
                                        ...
                                        @if (count($targets) > 0)
                                            <div class="flex flex-wrap gap-1">
                                                @foreach ($targets as $target)
                                                    <span
                                                        class="inline-flex items-center text-[10px] font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md">
                                                        {{ $target }}
                                                    </span>
                                                @endforeach
                                            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($tx->status === 'pending')
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 px-2.5 py-1 rounded-full">
                                                <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
                                                Pending
                                            </span>
                                        @elseif($tx->status === 'paid')
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-full">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                                Lunas
                                            </span>
                                        @elseif($tx->status === 'shipped')
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full">
                                                <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span>
                                                Dikirim
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 px-2.5 py-1 rounded-full">
                                    ...
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        <a class="inline-flex items-center gap-1 text-xs font-bold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 border border-emerald-100 dark:border-emerald-900 bg-emerald-50/50 dark:bg-emerald-950/10 px-3 py-1.5 rounded-lg transition-colors"
                                            href="{{ route('admin.transaction.show', $tx->id) }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-slate-400 dark:text-slate-500">
                                        <i data-lucide="inbox"
                                            class="w-10 h-10 mx-auto text-slate-300 dark:text-slate-600 mb-3"></i>
        ...
        {{-- <div id="expiry-alert" class="space-y-6">
            <div
                ...
                <div class="flex items-center gap-2.5 pb-4 mb-4 border-b border-slate-100 dark:border-slate-700">
                    <div
                        class="w-8 h-8 rounded-lg bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 flex items-center justify-center">
                ...
                <div class="space-y-4">
                    @forelse($expiringProductsList as $prod)
                        ...
                        @endphp
                        <div
                            class="flex gap-3 p-3.5 rounded-xl border {{ $isExpired ? 'border-rose-100 bg-rose-50/50 dark:border-rose-950/20 dark:bg-rose-950/5' : 'border-amber-100 bg-amber-50/30 dark:border-amber-950/10 dark:bg-amber-950/5' }} transition-colors">
                            ...
                            <div class="min-w-0 flex-1">
                                <h4 class="text-xs font-bold text-slate-900 dark:text-white truncate">{{ $prod->name }}
                                </h4>
                                <p class="text-[10px] text-slate-400 font-semibold mt-0.5">Batch:
                                    {{ $prod->batch_number ?? '-' }} | Izin: {{ $prod->registration_number ?? '-' }}</p>
                                ...
                                @if ($prod->target_animals)
                                    <div class="flex flex-wrap gap-1 mt-1.5">
                                        @foreach (explode(', ', $prod->target_animals) as $tag)
                                            <span
                                                class="text-[9px] font-bold bg-white/80 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded border border-slate-100 dark:border-slate-700">
                                                {{ $tag }}
                                            </span>
                                        @endforeach
                                    </div>
                                ...
                                <div
                                    class="flex items-center justify-between mt-2.5 pt-2 border-t border-slate-100 dark:border-slate-700/50">
                                    <span class="text-[10px] font-medium text-slate-400">Tgl:
                                        {{ $prod->expiry_date->format('d M Y') }}</span>
                                    @if ($isExpired)
                                    ...
                                    @else
                                        <span
                                            class="text-[10px] font-bold {{ $daysLeft <= 30 ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400' }}">
                                            {{ $daysLeft }} hari lagi
                                        </span>
                                    @endif
                                </div>
                    ...
                    @empty
                        <div
                            class="p-6 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 text-center text-slate-400 dark:text-slate-500">
                ...
                <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 text-center">
                    <a href="{{ route('admin.product.index') }}"
                        class="text-[11px] font-bold text-slate-500 hover:text-emerald-600 dark:text-slate-400 dark:hover:text-emerald-400 flex items-center justify-center gap-1.5">
            ...
            <div
                class="bg-gradient-to-tr from-slate-900 to-slate-800 text-white rounded-2xl p-6 shadow-md relative overflow-hidden group">
                ...
                <p class="text-xs text-slate-300 leading-relaxed">
                    Berdasarkan regulasi Kementerian Pertanian RI, setiap obat hewan yang disalurkan wajib memiliki nomor
                    batch yang terdaftar dan nomor pendaftaran BPOM/Kementan RI. Untuk <strong>Golongan Obat Keras / Resep
                        Dokter Hewan</strong>, pastikan resep diunggah dan diverifikasi sebelum pengiriman.
                </p>
                <div class="mt-4 flex gap-2">
                    <span class="text-[10px] font-bold bg-slate-800 text-slate-300 px-2.5 py-1 rounded-lg">Kementan
                        RI</span>
