    ...
    <main class="max-w-[1280px] mx-auto px-margin-mobile md:px-margin-desktop py-8 space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7">
                <div class="bg-white rounded-2xl overflow-hidden aspect-[4/3] flex items-center justify-center border border-outline-variant shadow-sm">
                ...
                <div class="bg-white rounded-2xl p-6 border border-outline-variant shadow-sm space-y-4">
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-800 leading-tight flex items-center gap-2">
                        ...
                        @if($product->needs_prescription)
                            <span class="inline-flex items-center text-[10px] font-extrabold bg-rose-50 text-rose-600 px-2 py-0.5 rounded border border-rose-100" title="Wajib Resep Dokter Hewan">
                                R/
                            </span>
                        @endif
                    </h1>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex text-secondary">
                            @for($i = 1; $i <= 5; $i++)
                                <span class="material-symbols-outlined text-[16px]" style="font-variation-settings: 'FILL' {{ $i <= round($product->rating) ? 1 : 0 }};">star</span>
                            @endfor
                        </div>
                        <span class="text-slate-400 text-xs font-semibold">{{ number_format($product->rating, 1) }} &bull; {{ $product->reviews()->count() }} ulasan</span>
                    </div>

                    <!-- Price -->
                    <div class="pt-2 border-t border-slate-50">
                        <p class="text-3xl font-extrabold text-primary">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                    </div>

                    <!-- Mini Info -->
                    <div class="text-xs text-slate-500 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-1">
                    ...
                    <div class="space-y-3">
                        ...
                        <form action="{{ route('cart.add') }}" method="POST" class="flex flex-col sm:flex-row gap-3 w-full">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $product->id }}" />
                            
                            <!-- Quantity counter -->
                            <div class="flex items-center border border-slate-200 rounded-xl overflow-hidden h-12 bg-slate-50/50 justify-between px-2 w-full sm:w-32">
                                <button type="button" class="w-8 h-8 rounded-lg hover:bg-slate-100 font-extrabold flex items-center justify-center text-slate-600 transition-colors qty-min">-</button>
                                <input type="number" name="quantity" class="w-10 text-center font-bold text-sm qty-val border-none focus:ring-0 p-0 bg-transparent text-slate-800" value="1" min="1" max="{{ $product->stock }}" readonly />
                                <button type="button" class="w-8 h-8 rounded-lg hover:bg-slate-100 font-extrabold flex items-center justify-center text-slate-600 transition-colors qty-plus" {{ $product->stock <= 1 ? 'disabled' : '' }}>+</button>
                            </div>

                            <!-- Buy Button -->
                            @if($product->stock > 0)
                                <button type="submit" class="flex-1 bg-primary text-white rounded-xl font-bold text-sm h-12 flex items-center justify-center gap-2 hover:bg-primary/95 active:scale-98 transition-all shadow-md">
                                    <span class="material-symbols-outlined text-[18px]">shopping_cart</span>
                                    Tambah ke Keranjang
                                </button>
                            @else
                                <button type="button" disabled class="flex-1 bg-slate-100 text-slate-400 rounded-xl font-bold text-sm h-12 flex items-center justify-center gap-2 cursor-not-allowed">
                                    <span class="material-symbols-outlined text-[18px]">shopping_cart</span>
                                    Stok Habis
                                </button>
                            @endif
                    ...
                    <div class="flex items-center gap-4 text-[10px] text-slate-400 font-semibold pt-1 border-t border-slate-50">
        ...
        <div class="bg-white rounded-2xl border border-outline-variant shadow-sm overflow-hidden mt-8">
            ...
            <div class="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50">
                <nav class="flex space-x-6 px-6" aria-label="Tabs">
                    <button type="button" class="desc-tab-link border-emerald-500 text-emerald-600 px-1 py-4 border-b-2 font-bold text-sm select-none cursor-pointer focus:outline-none transition-colors" data-target="#tab-deskripsi">
                        Deskripsi
                    </button>
                    <button type="button" class="desc-tab-link border-transparent text-slate-400 hover:text-slate-600 px-1 py-4 border-b-2 font-semibold text-sm select-none cursor-pointer focus:outline-none transition-colors" data-target="#tab-komposisi">
                        Kandungan &amp; Komposisi
                    </button>
                    <button type="button" class="desc-tab-link border-transparent text-slate-400 hover:text-slate-600 px-1 py-4 border-b-2 font-semibold text-sm select-none cursor-pointer focus:outline-none transition-colors" data-target="#tab-aturan">
                        Indikasi &amp; Aturan Pakai
                    </button>
                    <button type="button" class="desc-tab-link border-transparent text-slate-400 hover:text-slate-600 px-1 py-4 border-b-2 font-semibold text-sm select-none cursor-pointer focus:outline-none transition-colors" data-target="#tab-dosis-hewan">
            ...
            <div class="p-6 text-sm text-slate-600 dark:text-slate-300 leading-relaxed min-h-[160px]">
                ...
                <div id="tab-komposisi" class="desc-tab-pane space-y-3 hidden">
                    ...
                    @else
                        <p class="text-slate-400 italic">Informasi kandungan bahan aktif tidak didetailkan secara spesifik.</p>
                    @endif
                </div>
                ...
                <div id="tab-aturan" class="desc-tab-pane space-y-3 hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block">Indikasi Medis</span>
                            <p class="text-slate-600 bg-slate-50 p-3.5 rounded-xl border border-slate-100">{{ $product->indication ?? 'Diformulasikan untuk spesies target.' }}</p>
                        </div>
                        <div class="space-y-1.5">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block">Aturan Pakai &amp; Administrasi</span>
                            <p class="text-slate-600 bg-slate-50 p-3.5 rounded-xl border border-slate-100">{{ $product->dosage_guidelines ?? 'Gunakan sesuai dosis label kemasan.' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div id="tab-dosis-hewan" class="desc-tab-pane space-y-3 hidden">
                    <h4 class="font-bold text-slate-800 dark:text-white text-base">Catatan Penyaluran &amp; Dosis Spesies</h4>
                ...
                <div class="bg-white p-6 rounded-2xl border border-outline-variant shadow-sm space-y-4">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-1">{{ $product->reviews()->count() }} ulasan dari pelanggan</p>
                        </div>
                    </div>
                </div>
                ...
                @if(Auth::check())
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-outline-variant shadow-sm space-y-4">
                        ...
                        <form action="{{ route('review.store', $product->id) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Rating Bintang</label>
                                <select name="rating" required class="w-full bg-slate-50 text-sm py-2.5 px-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Komentar Ulasan</label>
                                <textarea name="comment" rows="3" required placeholder="Tuliskan pengalaman Anda menggunakan obat hewan ini..." class="w-full bg-slate-50 text-sm py-2.5 px-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent outline-none"></textarea>
                            </div>
                            <button type="submit" class="w-full bg-primary text-white py-2.5 rounded-xl font-bold text-xs hover:bg-primary/95 active:scale-98 transition-all shadow-sm">
                                Kirim Ulasan
                            </button>
                        </form>
                    </div>
            ...
            <div class="lg:col-span-8">
                <div class="bg-white p-6 rounded-2xl border border-outline-variant shadow-sm space-y-6">
                    ...
                    <div class="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                        ...
                        @empty
                            <p class="text-center text-slate-400 py-12 text-sm italic">Belum ada ulasan untuk produk obat ini.</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            tabLinks.forEach(link => {
                link.addEventListener('click', function() {
                    ...
                    tabLinks.forEach(item => {
                        item.classList.remove('border-emerald-500', 'text-emerald-600', 'font-bold');
